...
class ProductuiController extends GetxController {
  ...
  final RxList<Map<String, dynamic>> childProducts = <Map<String, dynamic>>[].obs;
  final RxString selectedSlug = ''.obs;
  final RxList<Map<String, dynamic>> activeVariant = <Map<String, dynamic>>[].obs;
  ...
  void _initializeSelectedSlug() {
    if (selectedVariants.containsKey(productId) && selectedVariants[productId]!.isNotEmpty) {
  ...
  // Getter for images
  List get images => activeVariant.isNotEmpty
      ? activeVariant[0]['images'] ?? []
      : [];

  // Getter for product title
  String get productTitle => activeVariant.isNotEmpty
  ...
  List<Map<String, dynamic>> get combinedMetal {
    ...
    for (final item in childProducts) {
      final metalStamps = (item['metalStamp'] as List? ?? []).cast<Map<String, dynamic>>();
      final metalTypes = (item['metalType'] as List? ?? []).cast<Map<String, dynamic>>();
  ...
  List<Map<String, dynamic>> get caratOptions {
    ...
    for (final child in childProducts) {
      final metalStamps = (child['metalStamp'] as List? ?? []).cast<Map<String, dynamic>>();
  ...
  bool get hasVariantData => childProducts.isNotEmpty && activeVariant.isNotEmpty;
}